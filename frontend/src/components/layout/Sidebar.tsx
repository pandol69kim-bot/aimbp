...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed left-0 top-0 z-50 flex h-full flex-col border-r border-white/10 bg-dark-900/80 backdrop-blur-md transition-transform duration-300 ease-in-out lg:static lg:translate-x-0',
          isMobile ? (isOpen ? 'translate-x-0 w-64' : '-translate-x-full w-64') : 'w-64'
        )}
      >
        {/* Logo */}
        <div className="flex items-center justify-between gap-3 px-6 py-5 border-b border-white/10">
          <div className="flex items-center gap-3 flex-1">
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-primary-600 to-accent-600 shrink-0">
          ...
          {isMobile && (
            <button
              onClick={closeSidebar}
              className="lg:hidden flex h-8 w-8 items-center justify-center rounded-lg hover:bg-white/10 transition-colors"
            >
